    ...
    void distribute_modules() {
        ...
        for (int i = 0; i < this->used_count; i++) {
            ...
            for (int j = 0; j < nodes_modules.size(); j++) {
                ...
                if (i == this->my_rank) {
                    this->mpi_manager_.add_new_module(mod->get_name(), mod->get_pla(),
                                                      this->my_rank, mod->get_var_count(),
                                                      mod->get_function_column());
                    // this->my_instructions =
                    // this->module_manager_.get_instructions_for_process(i);
                    continue;
                }

                this->mpi_manager_.send_module_info(
                    mod, /*this->module_manager_.get_instructions_for_process(i),*/ i);
            }

            if (i == 0) {
                this->my_instructions = this->module_manager_.get_instructions_for_process(i);
            } else {
                mpi_communicator::send_string(this->module_manager_.get_instructions_for_process(i),
                                              i);
            }
        }
    }
  ...
  public:
    ...
    void process_information() override {
        ...
        if (this->assigned_count > 0) {
            this->mpi_manager_.recieve_my_modules(this->assigned_count, this->my_rank,
                                                  this->my_instructions);
        }
    }
};
