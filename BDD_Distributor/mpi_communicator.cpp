...
void
mpi_communicator::send_int(int message, int recievers_rank) {
    MPI_Send(&message, 1, MPI_INT, recievers_rank, 0, MPI_COMM_WORLD);
}

int
mpi_communicator::recv_int(int sender_rank) {
    int message;
...
void
mpi_communicator::send_double(double message, int recievers_rank) {
    MPI_Send(&message, 1, MPI_DOUBLE, recievers_rank, 0, MPI_COMM_WORLD);
}

double
mpi_communicator::recv_double(int sender_rank) {
    double message;
...
void
mpi_communicator::send_string(std::string message, int recievers_rank) {
...
std::string
mpi_communicator::recv_string(int sender_rank) {
    int size;
    // char* message;
...
void
mpi_communicator::scatter_ints(int* message, int* recieve_buffer) {
    MPI_Scatter(message, 1, MPI_INT, recieve_buffer, 1, MPI_INT, 0, MPI_COMM_WORLD);
}

void
mpi_communicator::gather_doubles(double* message, double* recieve_buffer) {
